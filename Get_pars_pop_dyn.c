...
extern int SCORE_KL;
extern float REG_INI;
extern float N_stab_thr;
...
Read_parameters(char *FILE_IN,
		...
		int *MEANFIELD,  int *MF_COMP, int *STR_COMP,
	     ...
	     char *dir_out)
{
  ...
  if(FILE_IN[0]!='\0'){
    ...
    r=Read_parameters(FILE_IN, file_pdb, chain, file_pdb_list, pdb_dir, 
		      ...
		      MEANFIELD, MF_COMP, STR_COMP, OPT_REG, SCORE_CV, REG, 
  ...
  for(j=1; j<argc; j++){
    ...
    }else if(strncmp(argv[j], "-opt_KL", 7)==0){
      SCORE_KL=1;
    }else if(strncmp(argv[j], "-stab_thr", 7)==0){
      j++; sscanf(argv[j], "%f", &N_stab_thr);
    }else if(strncmp(argv[j], "-reg", 4)==0){
      j++; sscanf(argv[j], "%f", &REG_INI);
   }else if(strcmp(argv[j], "-nostr")==0){
      *STR_COMP=0; // Do not compute structural deformation with TNM
    }else if(strncmp(argv[j], "-tnm", 4)==0){
      j++; sscanf(argv[j], "%s", tnm);
    }else if(strncmp(argv[j], "-iwt", 4)==0){
      j++; sscanf(argv[j], "%d", IWT);
		    ...
		    char *dir_out)
{
  ...
  while(fgets(string, sizeof(string), file_in)!=NULL){
    ...
    }else if(strncmp(string, "STR_COMP", 8)==0){
      sscanf(string+9,"%d", STR_COMP);
...
void help(char *prog){
  ...
  printf("  -opt_KL           # Optimize KL divergence instead of likel.\n");
  printf("  -reg  <Regularization parameter>\n");
  printf("  -stab_thr  <Min. number of stable seqs for accepting a pdb>\n");
  printf("  -nostr  Do not predict structural deformation with TNM (slow)\n");
  ...
  printf("Simulations of protein evolution:\n");
  printf("  -it  <IT_MAX>     # Number of substitutions per site\n");

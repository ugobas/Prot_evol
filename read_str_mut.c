...
int Read_str_mut(float **Str_mut, char *file_str_mut,
		 struct protein target, int end_dom, int IWT)
{
  if(file_str_mut[0]=='\0')return(0);
  ...
  for(int i=0; i<end_dom; i++){
    float *Si=Str_mut[i];
    for(int a=0; a<20; a++)Si[a]=0;
  }

  char string[9000]; 
  int wrong_thr=5, k=0, wrong=0, L=target.L_PDB, imax=0;
  ...
  if(k!=L){
    printf("WARNING, file %s contains %d lines but %d residues in prot\n",
  ...
  int num[imax]; for(k=0; k<imax; k++)num[k]=0;
  ...
  int KWT=IWT; if(KWT>19){KWT=19;}else if(KWT<0){KWT=0;}
  for(int i=0; i<imax; i++){
    float *Str_i=Str_mut[i];
    if(num[i]>1)for(a=0; a<20; a++)Str_mut[i][a]/=num[i];
    if(KWT){
      f_sort(Str_i, 20, i_rank);
      float Y=0; for(a=0; a<IWT; a++)Y+=Str_i[i_rank[Na1-a]];
      int wt=aseq[i]; Str_i[wt]=Y/IWT;
    }
  }
  return(1);
}
...
int Read_str_mut_all(float ***Str_mut, char *file_str_mut,
		     struct protein target, int end_dom)
{
  if(file_str_mut[0]=='\0')return(0);
  FILE *file_in=fopen(file_str_mut, "r");
  if(file_in==NULL){
  ...
  char string[9000];
  int wrong_thr=10, k=0, wrong=0, L=target.L_PDB, imax=0;
  short *aseq=target.aa_seq;
  while(fgets(string, sizeof(string), file_in)!=NULL){
    if(string[0]=='#')continue;
    if(k>=L){

		 ...
		 int L, int Naa)
{
  ...
  float Lam[2]; Lam[0]=Lambda; Lam[1]=0;
  ...
  Compute_score(&nat_opt, P_MF_ia, Lam, f_reg_ia, f_msa_ia, wi, L, Naa, 1);
  ...
  Compute_score(&all_opt, P_MF_ia, Lam, f_reg_ia, f_msa_ia, wi, L, Naa, 1);
		   ...
		   int L, int Naa, int type, char *name)
{
  ...
  Compute_score(&res, P_MF_ia, Lam, f_reg_ia, f_msa_ia, wi, L, Naa, 1);
  fprintf(file_out, "%.2f %6.1f %.4f %.4f %.4f %.4f %.3f %.2f %d\n",
	  Lambda, res.DG,
	  res.score_reg, res.KL_mod, res.KL_reg, res.lik_reg,
	  res.h, res.Tf, res.conv);
  score_k[1]=res.score_reg; Lambda_k[1]=Lambda;
  *opt_res=res; opt_res->Lambda[0]=Lambda; opt_res->Lambda[1]=0;
  ...
  for(int iter=0; iter<100; iter++){
    ...
    for(k=0; k<=2; k+=2){
      ...
      score_k[k]=res.score_reg;
      if((res.entropy<=Entropy_min)||(isnan(res.score_reg)))end=1;
      if(res.score_reg > opt_res->score_reg){
	*opt_res=res; opt_res->Lambda[0]=Lambda;
    ...
    fprintf(file_out, "%.2f %6.1f %.4f %.4f %.4f %.4f %.3f %.2f %d\n",
	    ...
	    res.h, res.Tf, res.conv);
    Lambda_k[1]=Lambda;
    score_k[1]=res.score_reg;
    if((res.entropy<=Entropy_min)||(isnan(res.score_reg)))break;
    if(res.score_reg > opt_res->score_reg){
      float score_old=opt_res->score_reg;
      *opt_res=res; opt_res->Lambda[0]=Lambda;
      Copy_P(P_opt_ia, P_MF_ia, L, Naa); 
      if(fabs(res.score_reg-score_old)<eps)break;
      n_down=0;
    }else{
      n_down++;
    }
  ...
  printf("Maximum score %s model: %.2f optimal Lambda=%.3f\n",
	 name, opt_res->score_reg, opt_res->Lambda[0]);
  char label[10];
  ...
  fprintf(file_out, "Lambda= %.3f lik= %.4f DG= %.1f h=%.3f\n",
	  opt_res->Lambda[0], opt_res->score_reg, opt_res->DG, opt_res->h);
		    ...
		    FILE *file_summary, int L, int Naa)
{
  ...
  /*if(GET_DG_OPT){
    ...
    if(fabs(all_opt.DG-DG_OPT) < dDG_opt){
      ...
      Lam[0]=Lambda;
      Compute_score(&tmp, P_MF_ia, Lam, L, f_reg_ia, f_msa_ia, wi);
...
void Print_results(struct MF_results res, char *what, FILE *file_out)
{
  ...
  fprintf(file_out, "%s\t"
	  "%.4f\t%.4f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.4f\t%.2f\t%.3f\n",
	  what, res.Lambda[0], res.Lambda[1], res.lik_reg, res.KL_mod,
	  res.KL_reg, -res.score_reg, res.KL_mut, res.entropy,
	  res.DG/res.L, res.Tf, res.h);
}
  ...
  while( (((DG_opt->DG)>0)||((DG_opt->DG)<2*DG_OPT))&&(iter<IT_MAX)){
    ...
    if(ddG > *dDG_opt){
      *dDG_opt=ddG;
      ...
      //tmp.lik=Compute_lik(P_MF_ia, L, f_reg_ia);
      Lam[0]=Lambda;
      Compute_score(&tmp, P_MF_ia, Lam, L, f_reg_ia, wi);
      tmp.h=Hydro_ave(P_MF_ia, hydro, L);
      *DG_opt=tmp; //Copy_vars(DG_opt, &tmp);
    }
  }
...
float Hydro_ave(float **P_ia, float *hydro, float *wi, int L){
  int i, a; double sum=0, h;
  for(i=0; i<L; i++){
    float *P_a=P_ia[i];
    h=0; for(a=0; a<20; a++)h+=P_a[a]*hydro[a];
    if(wi){sum+=wi[i]*h;}else{sum+=h;}
  }
  if(wi==NULL){sum/=L;}
  return(sum);
}

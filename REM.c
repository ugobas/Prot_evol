...
void Rescale_REM(int L_pdb, char *nameout, int n_LEN);
		      ...
		      char *file_str)
{
  ...
  if(init_REM){
    N_PROT=Initialize_REM(file_str, nameout);
    E->L=0; E->c1U1=NULL;
    init_REM=0;
  }
  if(L != LEN){
    printf("Rescaling REM\n");
    Rescale_REM(L, nameout, n_LEN);
    LEN=L; n_LEN++;
    if(E->c1U1)free(E->c1U1);
    E->c1U1=NULL;
  }
  if(E->c1U1==NULL){
...
void Rescale_REM(int L, char *nameout, int n_LEN)
			 ...
			 int L)*/
{
  ...
  for(int l=0; l<L; l++){
    Cont_L[l] =0; Cont_1L[l]=0;
    Cont_2L[l]=0; Cont_3L[l]=0;
    C2_L[l]=0; C3_L[l]=0;
  }
  ...
  for(d=0; d<=DTAIL_MAX; d++){
    ...
    nc_L[d]=0; nc2_L[d]=0;
    nc_Nc_L[d]=0; nc_Nc2_L[d]=0;
    for(int l=0; l<L; l++){
      nc_nc_L[d][l]=0; nc_nc_Nc_L[d][l]=0;
    }
  }
...
float Compute_Tfreeze(struct REM *E){
  ...
  float Tf;
  if(E->REM<=1){return(0);}
  if(isnan(E->E2) || E->E2<=0){Tf=0.01;}
  else if(E->S_C<=0){Tf=100;}
  else{Tf=sqrt(E->E2/(2*E->S_C));}
...
float DeltaG(float E_nat, float G_misf, float S_U)
{
  ...
  // Note that G_U/T =-S_U
  float dG=G_misf+S_U, dU=E_nat+S_U, dm=E_nat-G_misf;
  //printf("### dG= %.3g dU= %.3g dm= %.3g\n", dG, dU, dm);
  if(abs(dG)>20 || abs(dU)>20 || abs(dm)>20){ 
    if(dG>0){return(dU);}
    else{return(dm);}
  }
  float DeltaG_overT=log(exp(dU)+exp(dm));
  //printf("### dG= %.3g\n", DeltaG_overT);
  return(DeltaG_overT);
}

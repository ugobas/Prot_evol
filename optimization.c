...
float Find_max_quad(float x1, float x2, float x3,
		    ...
		    float MIN, float MAX)
{
  //printf("x1= %.3g x2= %.3g x3= %.3g\n", x1, x2, x3);
  //printf("y1= %.3g y2= %.3g y3= %.3g\n", y1, y2, y3);
  ...
  float yprime_3=((y3-y2)/(x3-x2));
  if(yprime_1 == yprime_3){
  ...
  float a=(yprime_1-yprime_3)/(x1-x3);
  if(a>=0 || isnan(a)){
  ...
  float x0=0.5*(x1+x2)-yprime_1/(2*a);
  //printf("a= %.2f x0= %.2f\n", a, x0);
